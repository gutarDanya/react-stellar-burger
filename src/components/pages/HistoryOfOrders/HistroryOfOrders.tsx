...
import { WS_CONNECTING, WS_DISCONNECT } from "../../../services/actions/WSHistoryAction";
    ...
    return (
        <div className={`${styles.container} custom-scroll`}>
            {orders && orders.length > 0 && orders.map((order: any) => {
                return (
                    <Order id={order._id} title={order.name} ingredients={order.ingredients} date={order.updatedAt} numbers={order.number} />
                )
            } )}
        </div>
    )
